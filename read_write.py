    ...
    def validate2_fail_regs(fail_regs,alg,flg,Fail,auto,is_cont,detections):
        ...
        for fail_field_name in fail_regs:
            ...
            if int(repr(num2print)[-1]) == 0:
                ...
                (alg,flg) = dump.export('store_fail',f'Pass:{Pass}')
                (alg,flg) = dump.export('store_fail',f'Fail:{Fail}')
                (alg,flg) = dump.export('store_fail',f'Unknown:{Unknown}')
                (alg,flg) = dump.export('store_fail',f'Error:{Error}')
                (alg,flg) = dump.export('store_fail',f'Hang:{Hang}')
                fail_rowdl=[]
                fail_x=[]
            num+=1
            if 'hang' in fail_reason:
